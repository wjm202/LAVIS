...
function _set_params(){
    model_item=${1:-"blip2-opt2.7b-stage2"}        # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
    ...
    max_epochs=${7:-"10"}                 # （可选）需保证模型执行时间在5分钟内，需要修改代码提前中断的直接提PR 合入套件  或是max_epoch
    num_workers=${8:-"3"}                # (可选)
...
function _analysis_log(){
    python benchmark/analysis_log.py ${model_item} ${log_file} ${speed_log_file} ${device_num} ${base_batch_size} ${fp_item} ${run_process_type}
}
