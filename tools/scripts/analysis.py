...
def parse_args():
    ...
    parser.add_argument(
        '--is_large_model', type=str, default=False, help='True for large model, False for small model')
...
class TimeAnalyzer(object):
    ...
    def analysis_last(self, base_batch_size, gpu_num=1, skip_steps=0, mode=-1, unit=None):
        if base_batch_size <= 0:
            print("base_batch_size should larger than 0.")
            return 0, ''

        if len(self.records) < 1:  # No records
            print("ERROR!!! no logs printed")
            return 0, ''

        try:
            result_records = self.records[-1]
        except Exception:
            print("no records")
            return 0, ''

        fps, fps_unit = self._get_fps(mode, base_batch_size, gpu_num, result_records, unit)
        # fps_skipped, _ = self._get_fps(mode, base_batch_size, gpu_num, avg_of_records_skipped, unit)
        Fips, _ = self._get_fps(mode, base_batch_size, gpu_num, result_records, unit)

        if mode == -1:
            print("\tIPS: %.3f %s" % (result_records, fps_unit))
            print("\tFPS: %.3f %s" % (fps, fps_unit))
        elif mode == 1 or mode == 3:
            print("\tIPS: %.3f steps/s" % result_records)
            print("\tFPS: %.3f %s" % (fps, fps_unit))
        elif mode == 0 or mode == 2:
            print("\tAvg: %.3f s/step" % result_records)
            print("\tFPS: %.3f %s" % (fps, fps_unit))
        return round(result_records, 3), fps_unit
...
class LossAnalyzer(object):
    ...
    def get_loss_avg(self):
        # for CE loss
        if os.path.exists(self.filename+"_2"):
            filelist=[self.filename, self.filename+"_2"]
        else:
            filelist=[self.filename]
        result_loss_avg_list = []
        for item in filelist:
            with open(item, "r") as f_object:
                lines = f_object.readlines()
                result_loss_list = []
                for line in lines:
                    if self.convergence_key in line and "avg_batch_cost" in line:
                        result_loss = 0
                        line = line.strip()
                        line_words = line.split(self.separator) if self.separator else line.split()
                        for i in range(len(line_words) - 1):
            ...
            result_loss_avg_list.append(sum(result_loss_list[-100:]) / 100)
        result_loss_avg = round(sum(result_loss_avg_list[:]) / len(result_loss_avg_list), 7)
...
if __name__ == "__main__":
    ...
    if re.match(r'DP.-MP.-PP.', args.run_mode) or 'DP_MoE_C' in args.run_mode or 'Sharding_MoE_C' in args.run_mode \
        or re.match(r'DP.-MP.', args.run_mode) or re.match(r'DP.-Sharding.', args.run_mode):
    ...
    is_large_model = args.is_large_model

    try:
        analyzer = TimeAnalyzer(args.filename, args.keyword, args.separator, args.position, args.range)
        if is_large_model == "True":
            run_info["ips"], run_info["speed_unit"] = analyzer.analysis_last(
            ...
            run_info["ips"], run_info["speed_unit"] = analyzer.analysis(
                ...
                mode=args.model_mode,
                unit=args.speed_unit)
        if "CE_ernie" in run_info["model_name"] or "CE_gpt_auto" in run_info["model_name"]:
            # for CE PaddleFleetX case loss
